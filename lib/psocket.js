...
PSocket.prototype.getSendSocketIdx=function(){
    if(this.connectedSockets.length<=0)
        return -1;
    var retIdx = this.lastSentSocketIdx;
    this.lastSentSocketIdx++;
    if(this.lastSentSocketIdx>=this.connectedSockets.length){
        this.lastSentSocketIdx=0;
    }
    return retIdx;
}
...
PSocket.prototype.emit=function(ev){
    ...
    var args = toArray(arguments);
    var curSockIdx= this.getSendSocketIdx();
    if(curSockIdx>=0){
        var socket = this.connectedSockets[curSockIdx];
        var ppacket = {packetId:this.getNextPacketId(),data:args};
        socket.emit.apply(socket, ['ppacket', ppacket]);
    }else{
        this.pendingPacketQueue.enqueue(['ppacket',ppacket]);
    }
    return this;
};
...
PSocket.prototype.onconnect=function(socket){
    ...
    if(idx === -1){
        ...
        while(!this.pendingPacketQueue.isEmpty()){
            socket.emit.apply(socket,this.pendingPacketQueue.dequeue());
        }
    }
};
/**
...
PSocket.prototype.onreceiveIdentity=function(socket){
    socket.emit('puuid',{data:this.uuid});
};
