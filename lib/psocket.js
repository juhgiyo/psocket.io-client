...
function PSocket(uri, opts){
    ...
    opts = opts || {};
    opts.socketCnt = opts.socketCnt || 20;
    opts.forceNew = true;
    ...
    for(var i=0 ;i <this.socketCnt; i++){
        ...
        socket.on('connect',function(){
            ...
            self.onerror(socket,  new Error('reconnect_error'));
...
PSocket.prototype.on=function(ev){
    ...
    for(var i=0;i<this.socketList.length;i++){
        this.socketList[i].on(ev, function(data){
            if(self.sequentialRecv){

            }else{
                emit.apply(self,[ev,data]);
            }
